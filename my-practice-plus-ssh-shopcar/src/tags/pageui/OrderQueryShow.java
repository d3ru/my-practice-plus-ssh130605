/**
 * 文件：orderQueryShow.java 说明：订单查询的显示ui 时间：08-05-19 编写：tarena
 */
package tags.pageui;
...
public class OrderQueryShow extends BodyTagSupport
{
	 ...
	 * 
	 */
	private static final long serialVersionUID = -3841241145718033928L;
	...
	/**
	 * 说明：订单查询 简单分页技术的实现,此处由于时间关系,没有将分页封装成类 在oracle数据库中查询结果的行号使用伪列ROWNUM表示（从1开始）。 例如select * from employee where rownum<10
	 * 返回前10条记录。但注意rownum是在查询之后排序之前赋值的 mySQL可以使用LIMIT子句： select name, birthday from employee order by birthday LIMIT 99,20 limit
	 * 99,20 表示查询结果从99行后开始,返回20行的结果 即返回 100-119 DB2有rownumber()函数用于获取当前行数。
	 */
