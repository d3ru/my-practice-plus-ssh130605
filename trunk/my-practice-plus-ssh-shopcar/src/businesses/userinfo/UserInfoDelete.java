/**
 * 说明：用户注册信息注销 文件：userinfo/UserInfoDelete.java->userinfo/userinfodelete.jsp 时间：08-05-18 编写：tarena
 */
...
public class UserInfoDelete extends HttpServlet
{
	...
	/**
	 * 说明：用户信息注销 使用：userinfodelete.jsp?uid=uid 方法： 1.检测用户id为uid是否在表中存在 2.检测用户id为uid的交易记录表中是否有订单 3.将该用户的所有cookie置为无效
	 * 4.从session中删除用户登录的session 5.删除用户id为uid的交易记录表中的所有交易记录 6.从用户表中删除用户id为uid的用户
	 */
	@Override
