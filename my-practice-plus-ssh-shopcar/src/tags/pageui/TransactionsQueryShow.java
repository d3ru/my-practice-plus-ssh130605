		...
		try {
			...
			if(rs.next()){
				...
				while(rs.next()){
					...
					out.println("<td align='center' valign='middle'><div id='isSigned"+ttid+"'>"+(rs.getString(TableInfo.TRAN_tshiped).equals("1")?"<a href='javascript:productSigned(\"order/shipedchange.jsp?orderid="+ttid+"\","+ttid+")'>已发货</a>":"已签收")+"</div></td>");
					out.println("</tr>");
