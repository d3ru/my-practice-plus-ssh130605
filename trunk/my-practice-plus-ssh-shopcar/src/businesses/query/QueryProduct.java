/**
 * 文件：queryProduct.java 说明：实现查询及高级查询 时间：08-05-20 编写：tarena
 */
...
public class QueryProduct extends HttpServlet
{
	 ...
	 * 
	 */
	private static final long serialVersionUID = -7537757770681504421L;
		...
		try
		{
			...
			if (rss.next())
			{
				...
				while (rs.next())
				{
					...
					// out.println("<td align='center' valign='middle'>"+queryTools.getColumnValue(dataAccess,
					// TableInfo.TYPE_ptname, TableInfo.TABLE_ProductType,
					// TableInfo.TYPE_ptid+"="+rs.getString(TableInfo.PROT_ptypeid))+"</td>");
